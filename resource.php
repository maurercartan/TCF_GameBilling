<?php
    ...
    function D3JS_2($php_array,$key)
    {
        ...
        foreach($php_array as $value)
        {
            $links = $links."{source: '".$value."', target: '".$key."', type: 'licensing'},";
        }
        $links = trim($links,",");
        echo "var links = [".$links."];";
    ...
    function read_csv2dict($csvFile)
    {
        ...
        while (($data = fgetcsv($fp, 1000, ",")) !== FALSE) {
            ...
            $result[$data[1]][count($result[$data[1]])] = $data[0];
        }
        return $result;
    }
    ...
    function Select_JS($FormName,$SelectName1,$SelectName2,$Select_List)
    {
        ...
        if($_POST['resource2']!="")
        {
            echo "
                (function() {
                    var form = document.forms['".$FormName."'];
                    var rel = form.elements['".$SelectName2."'];
                    //rel.selectedIndex = 5;
                    //alert('".$_POST['resource2']."');
                    rel.value = '".$_POST['resource2']."';
                }());
            ";
        }
...
<?php
    ...
    echo "<div class='resourcesystem'>";
        ...
        echo "<div class='resourceform'>";
            ...
            echo "<option value='建築：祭壇等級' ".Keep_Select_Option($_POST['resource2'],'建築：祭壇等級').">建築：祭壇等級</option>";
            echo "<option value='PVE勝利次數' ".Keep_Select_Option($_POST['resource2'],'PVE勝利次數').">PVE勝利次數</option>";
            echo "<option value='建築：冒險傳送門等級' ".Keep_Select_Option($_POST['resource2'],'建築：冒險傳送門等級').">建築：冒險傳送門等級</option>";
            echo "<option value='建築：聯盟大廳等級' ".Keep_Select_Option($_POST['resource2'],'建築：聯盟大廳等級').">建築：聯盟大廳等級</option>";
            echo "<option value='主動發起PVE次數' ".Keep_Select_Option($_POST['resource2'],'主動發起PVE次數').">主動發起PVE次數</option>";
            echo "<option value='建築：要塞等級' ".Keep_Select_Option($_POST['resource2'],'建築：要塞等級').">建築：要塞等級</option>";
            echo "<option value='PVE次數' ".Keep_Select_Option($_POST['resource2'],'PVE次數').">PVE次數</option>";
            echo "<option value='科研：據點二' ".Keep_Select_Option($_POST['resource2'],'科研：據點二').">科研：據點二</option>";
            echo "<option value='建築：治療之泉等級' ".Keep_Select_Option($_POST['resource2'],'建築：治療之泉等級').">建築：治療之泉等級</option>";
            echo "<option value='科研：守護者' ".Keep_Select_Option($_POST['resource2'],'科研：守護者').">科研：守護者</option>";
            echo "<option value='科研：吟唱者' ".Keep_Select_Option($_POST['resource2'],'科研：吟唱者').">科研：吟唱者</option>";
            echo "<option value='科研：巨獸馴獸師' ".Keep_Select_Option($_POST['resource2'],'科研：巨獸馴獸師').">科研：巨獸馴獸師</option>";
            echo "<option value='建築：倉庫等級' ".Keep_Select_Option($_POST['resource2'],'建築：倉庫等級').">建築：倉庫等級</option>";
            echo "<option value='建築：智慧神廟等級' ".Keep_Select_Option($_POST['resource2'],'建築：智慧神廟等級').">建築：智慧神廟等級</option>";
            echo "</select>";
            echo "</div>";
    ...
    {
        $php_array = read_csv2dict("./Data/TT_TF/chinese_TT_3.csv");
        D3JS_2($php_array['建築：祭壇等級'],'建築：祭壇等級');
    }
